...
- [ ] **Step 1: Extract the tube kernel** into `src/render/tubeFlow.ts` as the single source (DRY — no duplication). **Move** `dirAt`, `pointAtU`, `drawTube`, and `TUBE_R` out of `tubes/view.ts` into `tubeFlow.ts` as exports: `drawTube(field, dots, dim, flow, lightAt, endU, nowSec, phase)` and helpers, where `field` is the `DotField` passed in (currently `drawTube` closes over `field`; parameterize it). Then update `tubes/view.ts` to import them from `tubeFlow.ts` and delete its local copies. Run the TUBES screenshot harness on `/5` to confirm no visual regression, and `npx tsc --noEmit`. HDT (Task 8 Step 2) imports the same module.
